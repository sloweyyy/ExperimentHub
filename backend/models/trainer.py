...
def train_epoch(model, train_loader, optimizer, epoch, job_id, status_callback):
    ...
    return {"loss": epoch_loss, "accuracy": epoch_accuracy, "time": epoch_time}
...
def validate(model, test_loader):
    ...
    return {"loss": test_loss, "accuracy": accuracy, "time": validation_time}
...
def train_model(model, job_id, config, status_callback=None):
    ...
    try:
        ...
        if status_callback:
            status_callback(
                {**epoch_status, "status": "completed", "final_results": final_results}
            )
